...
pep8: remember
	flake8 --max-complexity 12 --exit-zero *.py graphiteeagle/*.py tests/*.py

flake8: pep8

lint: remember
	pylint --msg-template="{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" \
		-r n *.py graphiteeagle/*.py tests/*.py || exit 0
